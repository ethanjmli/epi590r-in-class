...
# model with robust standard errors using {gtsummary}. Your job now is to do the
# same using broom::tidy(). You’ll need some extra packages, though, because if
# you look at the broom::tidy() documentation, it doesn’t say anything about adding
# robust standard errors. The answer, then, is usually to start Googling. I’ve done
# so for you and found this post (I highly recommend all of Andrew Heiss’s R and statistics explanations!).
# Use this as a guide to create a dataframe with the results from both models
# (you can use bind_rows() to combine them, as in the example).

logistic_model <- glm(glasses ~ eyesight_cat + sex_cat,
											data = nlsy, family = binomial(link="log"))

poisson_model.2 <- glm(glasses ~ eyesight_cat + sex_cat,
											 data = nlsy, family = poisson(link = "log"))

logistic_robust <- coeftest(logistic_model,
														vcov = vcovHC,
														type = "HC1")

poisson_robust <- coeftest(poisson_model.2,
													 vcov = vcovHC,
													 type = "HC1")

tidy_logistic <- tidy(logistic_robust, conf.int = TRUE)

tidy_poisson <- tidy(poisson_robust, conf.int = TRUE)


bind_rows(
	poisson = tidy_poisson,
	logistic = tidy_logistic,
	.id = "model") |>
	mutate(
		term = str_remove(term, model),
		term = ifelse(term == "", model, term))

#Return to your final project. You are going to need to do some type of modeling
#and create some tables and figures, so this is a good time to start playing around!
